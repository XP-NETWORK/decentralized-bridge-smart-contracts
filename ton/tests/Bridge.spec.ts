...
describe('Bridge', () => {
    ...
    beforeEach(async () => {
        ...
        bridge = blockchain.openContract(
            await Bridge.fromInit(
                BigInt(`0x${bootstrapValidator.publicKey.toString('hex')}`),
                bootstrapValidatorAddress.address,
                'TON',
            ),
        );
    ...
    //         await TestnetNftCollection.fromInit(deployer.address, beginCell().storeInt(1, 32).endCell(), {
    //             $$type: 'RoyaltyParams',
    //             denominator: 10n,
    ...
    it('should add validators', async () => {
        ...
        let signature = sign(beginCell().store(storeNewValidator(nv)).endCell().hash(), bootstrapValidator.secretKey);
    ...
    it('should blacklist validator', async () => {
        const newValidatorKeypair = testKey('newValidator');
        const wallet = WalletContractV4.create({ workchain: 0, publicKey: newValidatorKeypair.publicKey });
        const nv = {
            $$type: 'NewValidator',
        ...
        let bsignature1 = sign(
            beginCell()
                .storeInt(0xff, 64)
                .storeUint(BigInt(`0x${bootstrapValidator.publicKey.toString('hex')}`), 256)
                .endCell()
                .hash(),
            bootstrapValidator.secretKey,
        );
        let bsignature2 = sign(
            beginCell()
                .storeInt(0xff, 64)
                .storeUint(BigInt(`0x${bootstrapValidator.publicKey.toString('hex')}`), 256)
                .endCell()
                .hash(),
            newValidatorKeypair.secretKey,
        );
        const blacklisted = await bridge.send(
            deployer.getSender(),
            {
                value: toNano('0.05'),
            },
            {
                ...
                len: 2n,
                sigs: Dictionary.empty<bigint, SignerAndSignature>()
                    .set(0n, {
                        $$type: 'SignerAndSignature',
                        key: BigInt(`0x${bootstrapValidator.publicKey.toString('hex')}`),
                        signature: beginCell().storeBuffer(bsignature1).endCell(),
                    })
                    .set(1n, {
                        $$type: 'SignerAndSignature',
                        key: BigInt(`0x${newValidatorKeypair.publicKey.toString('hex')}`),
                        signature: beginCell().storeBuffer(bsignature2).endCell(),
                    }),
                validator: BigInt(`0x${bootstrapValidator.publicKey.toString('hex')}`),
            },
        );
        console.log(blacklisted.transactions.map(flattenTransaction));
        expect(await bridge.getValidator(BigInt(`0x${bootstrapValidator.publicKey.toString('hex')}`))).toBeNull();
    });
    ...
    it('should claim data', async () => {
        let cd: ClaimData = {
            ...
            data3: {
                ...
                sourceNftContractAddress: beginCell()
                    .storeSlice(beginCell().storeStringTail('0x0').endCell().asSlice())
                    .endCell(),
            },
            data4: {
                $$type: 'ClaimData4',
                newContent: beginCell().storeInt(0x01, 8).storeStringRefTail('https://example.org').endCell(),
                lockTxChain: 'BSC',
                royalty: {
                    $$type: 'RoyaltyParams',
                    denominator: 10000n,
        ...
        const signature = sign(beginCell().store(storeClaimData(cd)).endCell().hash(), bootstrapValidator.secretKey);

        const txns = await bridge.send(
            deployer.getSender(),
            {
                value: toNano('0.2'),
            },
...

        {
            let cd: ClaimData = {
                ...
                data3: {
                    ...
                    royaltyReceiver: deployer.address,
                    sourceNftContractAddress: beginCell()
                        .storeSlice(beginCell().storeStringTail('0x15').endCell().asSlice())
                        .endCell(),
                },
                data4: {
                    $$type: 'ClaimData4',
