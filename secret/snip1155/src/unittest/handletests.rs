...
//     curate.balances = vec![
//         TokenIdBalance { address: addr0.clone(), amount: Uint128::new(1) },
//         TokenIdBalance { address: addr1.clone(), amount: Uint128::new(1) },
//         ];
//     let mut msg = ExecuteMsg::CurateTokenIds{initial_tokens: vec![curate], memo: None, padding: None };
...
//     curate.token_info.token_config = TknConfig::default_nft();
//     curate.balances[0].amount = Uint128::new(2);
//     msg = ExecuteMsg::CurateTokenIds{initial_tokens: vec![curate], memo: None, padding: None };
...
//     info.sender = addr.c();
//     execute(deps.as_mut(), mock_env(), info.clone(), msg_change_metadata.clone())?;
...
//     info.sender = addr.a();
//     result = execute(deps.as_mut(), mock_env(), info.clone(), msg_change_metadata_nft2.clone());
...
//     // init addresses
//     let addr0 = Addr::unchecked("addr0".to_string()); let addr0_bin = to_binary(&addr0)?; let _addr0_u8 = addr0_bin.as_slice();
//     let addr1 = Addr::unchecked("addr1".to_string()); let addr1_bin = to_binary(&addr1)?; let addr1_u8 = addr1_bin.as_slice();
//     let addr2 = Addr::unchecked("addr2".to_string()); let addr2_bin = to_binary(&addr2)?;let addr2_u8 = addr2_bin.as_slice();
...
//     assert_eq!(perm_r(&deps.storage, &addr0, "0").load(addr1_u8)?.trfer_allowance_perm, Uint128::new(9));
//     assert_eq!(perm_r(&deps.storage, &addr0, "0").load(addr2_u8)?.trfer_allowance_perm, Uint128::new(5));
//     assert_eq!(perm_r(&deps.storage, &addr1, "0").load(addr1_u8)?.trfer_allowance_perm, Uint128::new(10));
...
//     let addr0 = Addr::unchecked("addr0".to_string()); let addr0_bin = to_binary(&addr0)?; let _addr0_u8 = addr0_bin.as_slice();
//     let addr1 = Addr::unchecked("addr1".to_string()); let addr1_bin = to_binary(&addr1)?; let addr1_u8 = addr1_bin.as_slice();
//     let addr2 = Addr::unchecked("addr2".to_string()); let addr2_bin = to_binary(&addr2)?;let _addr2_u8 = addr2_bin.as_slice();
...
//     info.sender = addr1.clone();
//     execute(deps.as_mut(), mock_env(), info.clone(), msg1_trnsf_0.clone())?;
//     assert_eq!(chk_bal(&deps.storage, "2", &addr2).unwrap(), Uint128::new(0));
//     assert_eq!(chk_bal(&deps.storage, "2", &addr0).unwrap(), Uint128::new(1));
...
//             assert_eq!(perm.unwrap().trfer_allowance_perm, Uint128::new(10))
//         }
//         _ => panic!("query error")
//     }
...
//     match q_answer {
//         QueryAnswer::Permission(perm) => {
//             assert_eq!(perm.unwrap().trfer_allowance_perm, Uint128::new(0))
//         }
//         _ => panic!("query error")
//     }
...
//     // create vk
//     let msg_create_vk = ExecuteMsg::CreateViewingKey { entropy: "foobar".to_string(), padding: None };
//     let mut info = mock_info(addr.a().as_str(), &[]);
...
//     // set vk
//     let msg_set_vk = ExecuteMsg::SetViewingKey { key: "foobar".to_string(), padding: None };
//     info.sender = addr.b();
...
//     assert!(RevokedPermits::is_permit_revoked(&deps.storage, PREFIX_REVOKED_PERMITS, addr.a().as_str(), "testpermit"));
...
//     info.sender = addr.b();
//     result = execute(deps.as_mut(), mock_env(), info.clone(), msg_add_curators.clone());
...
//     };
//     info.sender = addr.c();
//     result = execute(deps.as_mut(), mock_env(), info.clone(), msg_add_minter_c.clone());
...
//     let msg_remove_minter_c = ExecuteMsg::RemoveMinters { token_id: "test0".to_string(), remove_minters: vec![addr.c()], padding: None };
//     info.sender = addr.c();
//     result = execute(deps.as_mut(), mock_env(), info.clone(), msg_remove_minter_c.clone());
//     assert!(extract_error_msg(&result).contains("You need to be the admin to add or remove minters"));
...
//     let msg_change_admin = ExecuteMsg::ChangeAdmin { new_admin: addr.b(), padding: None };
//     let mut info = mock_info(addr.b().as_str(), &[]);
//     let result = execute(deps.as_mut(), mock_env(), info.clone(), msg_change_admin.clone());
//     assert!(extract_error_msg(&result).contains("This is an admin function"));
...
//     let msg_add_curators = ExecuteMsg::AddCurators { add_curators: vec![addr.b()], padding: None };
//     let result = execute(deps.as_mut(), mock_env(), info.clone(), msg_add_curators.clone());
//     assert!(extract_error_msg(&result).contains("This is an admin function"));
