...
use cosmwasm_std::{
    entry_point, to_binary, Addr, Binary, BlockInfo, Deps, Env, StdError, StdResult, Timestamp,
    Uint256,
};
use secret_toolkit::{
...
use crate::{
    msg::{Snip1155QueryAnswer, Snip1155QueryMsg, QueryWithPermit},
    state::{
        balances_r, blockinfo_r, contr_conf_r, get_receiver_hash,
        permissions::{
            list_owner_permission_keys, may_load_any_permission, Permission, PermissionKey,
        },
...
#[entry_point]
pub fn query(deps: Deps, _env: Env, msg: Snip1155QueryMsg) -> StdResult<Binary> {
    match msg {
...
fn viewing_keys_queries(deps: Deps, msg: Snip1155QueryMsg) -> StdResult<Binary> {
    ...
    for address in addresses {
        ...
        if result.is_ok() {
            return match msg {
                ...
                Snip1155QueryMsg::TokenIdPrivateInfo {
                    address, token_id, ..
...
fn query_token_id_public_info(deps: Deps, token_id: String) -> StdResult<Binary> {
    ...
    match tkn_info_op {
        ...
        Some(mut tkn_info) => {
            ...
            let response = Snip1155QueryAnswer::TokenIdPublicInfo {
...
fn query_token_id_private_info(deps: Deps, viewer: &Addr, token_id: String) -> StdResult<Binary> {
    ...
    let response = Snip1155QueryAnswer::TokenIdPrivateInfo {
        token_id_info: tkn_info,
        total_supply,
        owner,
    };
...
fn query_registered_code_hash(deps: Deps, contract: Addr) -> StdResult<Binary> {
    ...
    let response: Snip1155QueryAnswer = match may_hash_res {
        Some(hash_res) => Snip1155QueryAnswer::RegisteredCodeHash {
...
fn query_transactions(deps: Deps, account: &Addr, page: u32, page_size: u32) -> StdResult<Binary> {
    ...
    let response = Snip1155QueryAnswer::TransactionHistory { txs, total };
    to_binary(&response)
}
...
fn query_all_permissions(
    ...
    let response = Snip1155QueryAnswer::AllPermissions {
