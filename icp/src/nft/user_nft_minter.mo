...
shared(_init_msg) actor class UserNftMinter(_args : {
    name: Text;
    symbol: Text;
    owner: Principal;
}) = this {
  ...
  stable var icrc7_migration_state = ICRC7.init(
    ICRC7.initialState() , 
    #v0_1_0(#id), 
   ICRC7Default.defaultConfig(_args.owner, ?_args.name, ?_args.symbol),
    _args.owner);
  ...
  stable var icrc37_migration_state = ICRC37.init(
    ICRC37.initialState() , 
    #v0_1_0(#id), 
    ICRC37Default.defaultConfig(_args.owner), 
    _args.owner);
  ...
  stable var icrc3_migration_state = ICRC3.init(
    ICRC3.initialState() ,
    #v0_1_0(#id), 
    ICRC3Default.defaultConfig(_args.owner),
    init_msg.caller);
  ...
  public query func owner(): async Principal {
    return icrc7().get_collection_owner();
  };
  ...
  public shared(msg) func icrcX_mint(token_id: Nat, to: ICRC7.Account, md: Text) : async [ICRC7.SetNFTResult] {
    ...
    let tokens: ICRC7.SetNFTRequest = [{
      ...
      memo = null;
      metadata = CandyTypesLib.shareCandy(unstable);
      owner = ?to;
      token_id = token_id;
      override = false;
    }];
