...
describe("Bridge", async () => {
  ...
  const tokenId721 = aptosClient.generateRandomTokenId();
  const tokenId1155 = aptosClient.generateRandomTokenId();
  ...
  describe("After Initilazed", async () => {
    ...
    describe.skip("Add Validator", async () => {
      ...
      it("Should fail if public keys and signatures length is not equal", async () => {
        const newValidator = validator2PbK;
    ...
    describe("Lock", async () => {
      ...
      describe("Lock 721", async () => {
        before(async () => {
          try {
            ...
            const mintEvent = tx.events.find(
              (e) => e.type === "0x4::collection::Mint"
            );
        ...
        it("Should successfully locks nft", async () => {
          try {
            ...
            const nftCollectionTokens =
              responseBridge?.nft_collection_tokens.data;
            mintedTokenId = Number(
              nftCollectionTokens[nftCollectionTokens?.length - 1].key.token_id
            );
            assert.ok(true);
          } catch (error: any) {
            console.log({ error });
      ...
      describe("Lock 1155", async () => {
        ...
        it("Should fail if destination chain is APTOS", async () => {
          destinationChain = Buffer.from("APTOS");
          try {
            let commitedTransaction = await aptosClient.lock721(
              testAccount,
              new HexString(tokenAddress),
    ...
    describe("Claim", async () => {
      describe("Claim 721", async () => {
        ...
        before(() => {
          try {
            const cleanHexAddress = collectionAddress.startsWith("0x")
              ? collectionAddress.slice(2)
              : collectionAddress;
            ...
            claimData = {
              ...
              fee: CLAIM_FEE_20_APT, // in octas. 1 APT = 10 ^ 8 octas
              destinationChain: Buffer.from("BSC"),
              sourceChain: Buffer.from("APTOS"),
              sourceNftContractAddress:
                aptosClient.hexStringToUint8Array(cleanHexAddress),
        ...
        it.skip("Should be able to claim and mint new nft successfully ", async () => {
          ...
          try {
            ...
            if (
              !(
                nftOwnerBalanceAfterTx <=
                nftOwnerBalanceBeforeTx - claimData.fee
              )
            ) {
              assert.ok(false);
            }
            ...
            if (bridgeDataAfterTx) {
              ...
              successfullyProcessedClaims = successfullyProcessedClaims + 1;
            } else {
              assert.ok(false);
            }
          } catch (error: any) {
        ...
        it("Should be able to claim and unlock nft successfully ", async () => {
          claimData.fee = CLAIM_FEE_POINT_1_APT;
          claimData.tokenId = mintedTokenId;
      ...
      describe("Claim 1155", async () => {
        ...
        it.skip("Should be able to claim and mint new nft successfully ", async () => {
          claimData.fee = CLAIM_FEE_POINT_1_APT;
          claimData.tokenId = tokenId1155 + 1;
          claimData.name = `Bridge # ${tokenId1155 + 1}`;
        ...
        it("Should be able to claim and unlock nft successfully ", async () => {
          ...
          try {
            ...
            if (bridgeDataBeforeTx) {
              ...
              console.log("Bridge not intialized");
              assert.ok(false);
            }
    ...
    describe.skip("Claim Validator Reward", async () => {
      ...
      it("should fail if validator doesnot exist", async () => {
        try {
          const serializer = new BCS.Serializer();
          serializer.serializeFixedBytes(nftOwnerPbK); // hashing wrong data. validator1 instead of newValidator
          const msgHash = createHash("SHA256")
            .update(serializer.getBytes())
            .digest();

          const validatorSignature = await ed.sign(msgHash, nftOwnerPrK);
      ...
      it.skip("should fail if validator pending reward is zero", async () => {
        try {
          const serializer = new BCS.Serializer();
          serializer.serializeFixedBytes(validator2PbK);
          const msgHash = createHash("SHA256")
            .update(serializer.getBytes())
            .digest();
      ...
      it("should successfully transfer validator pending reward", async () => {
        try {
          ...
          const validator1Signature = await ed.sign(msgHash, validator1PrK);
          const validator2Signature = await ed.sign(msgHash, validator2PrK);
          const publicKeys = [validator1PbK, validator2PbK];
          ...
          const totalValidators = publicKeys.length;

          assert.ok(
            validatorBalanceAfterTx ===
              validatorBalanceBeforeTx +
                Math.trunc(
                  (CLAIM_FEE_POINT_1_APT * successfullyProcessedClaims) /
                    totalValidators
                )
          );
