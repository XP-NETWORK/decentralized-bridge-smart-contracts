module bridge::aptos_nft_bridge {
  ...
  struct Bridge has key {
    ...
    nft_collection_tokens: Table<CollectionNftObject, address>,
  ...
  public entry fun lock_721(
    ...
    // let nft_already_exists = table::contains(&mut bridge_data.nft_collection_tokens, CollectionNftObject { collection: collection_address, token_id: token_address});
    
    let nft_token_id: &u256;
    
    // if(nft_already_exists) {
    //   // if it does exist get the nft id from the struct.
    //   nft_token_id = table::borrow(&mut bridge_data.nft_collection_tokens, CollectionNftObject { collection: collection_address, nft_address: token_address})
    // } else {
  ...
  public entry fun lock_1155(
    ...
    // let nft_already_exists = table::contains(&mut bridge_data.nft_collection_tokens, CollectionNftObject { collection: collection_address, nft_address: token_address});
  ...
  public entry fun claim_721(
    ...
    // royalty_points_numerator: u64, // royalty =  (royalty_points_numerator / royalty_points_denominator) * 100. for 5% royalty set royalty_points_numerator = 500
    // royalty_points_denominator: u64, // should be 10000
    royalty_percentage: u64,
    royalty_payee_address: address,
    fee: u64,
    signatures: vector<vector<u8>>, 
    ...
    source_chain: vector<u8>,
    source_nft_contract_address: vector<u8>,
    token_id: u256, // we will use this to generate token name e.g. Collection # 1
    transaction_hash: vector<u8>,
    nft_type: vector<u8>,
    ...
    // } else {
      // TODO: get token address using sourceNftContractAddress and tokenId
      let token_exists = table::contains(
        &mut bridge_data.nft_collection_tokens, 
        CollectionNftObject { collection_address: source_nft_contract_address, token_id }
      );
      let token_address: &address = &@0x0; 
      // let resource_token_addr = token::create_token_address(&bridge_resource_addr, &name, &name);

      if(token_exists) {

        token_address = table::borrow(
          &mut bridge_data.nft_collection_tokens, 
          CollectionNftObject { collection_address: source_nft_contract_address, token_id }
        );
